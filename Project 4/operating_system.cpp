...
void Operating_System::read_configuration_file(ifstream& fin, const string& config_filepath, string& metadata_filepath) {
   ...
   */
   ...
   hardware.quantum_number = quantum;
...
void shortest_job_first() { //easier without PCB data
   int shortest_job, num_processes = 0;
   vector<int> process_number, tasks_per_process, starting_idx, ending_idx;
   vector<Instruction> = new_instr;
   
   //get number of processes in metadata file
   num_processes = 0;
   for(int i = 0; i < instructions.size(); i++) {
      if(instructions.at(i).metadata_code == 'A' && instructions.at(i).descriptor == "start" && instructions.at(i).run_cycles == 0) {
         num_processes++;
      }
   }

   //loop for number of processes
   for(int i = 0; i < num_processes; i++) {
      if(i == 0) {
         new_instr.push_back(instructions.at(0));
      ...
      for(int j = 0; j < instructions.size(); j++) {
         if(instructions.at(j).metadata_code == 'A' && instructions.at(j).descriptor == "start" && instructions.at(j).run_cycles == 0) {
            process_number.push_back(num_processes);
            num_processes++;
         }
      }
      ...
      for(int k = 0; k < instructions.size(); k++) {
         if(instructions.at(k).metadata_code == 'A' && instructions.at(k).descriptor == "start" && instructions.at(k).run_cycles == 0) {
            starting_idx.push_back(k);
            int numTasks = 0;
            int w = k + 1;
            
            while(instructions.at(w).metadata_code != 'A' && instructions.at(w).descriptor != "end" && instructions.at(w).run_cycles != 0) {
               numTasks++;
               w++;
            }
           
            tasks_per_process.push_back(numTasks);
            ending_idx.push_back(w);
         }
      }
      
      //assign arbitrary shortest job
      shortest_job = process_number[0];
      
      //loop through metadata to determine shortest job
      for(int i = 0; i < tasks_per_process.size(); i++) {
         if(tasks_per_process[i] < tasks_per_process[i - 1]) {
            shortest_job = process_number[i];
         }
      }
      
      //added current shortest process to new metadata vectors
      for(int i = starting_idx[shortest_job]; i <= ending_idx[shortest_job]; i++) {
         new_instr.push_back(instruction.at(i));
      }
      
      //erase loaded metadata
      instructions.erase(instructions.begin() + starting_idx[shortest_job], instructions.begin() + (ending_idx[shortest_job] + 1));
      ...
      //copy old instr to new instr
      instructions.clear();
      for(i = 0; i < instructions.size(); i++) {
         instructions.push_back(new_instr.at(i));
      }
      
      //clear temps
      new_instr.clear();
      process_number.clear();
      tasks_per_process.clear();
      starting_idx.clear();
      endingIndex.clear();
   }
}
...
int process_errors(const int& error, Operating_System& OS) {
   ...
   if(error == -9) {
      OS.print_log(true);         
      OS.file_log(true);
      cout << "ERROR CODE -8: INVALID SCHEDULING ALGORITHM" << endl;      
      return EXIT_FAILURE;
   }
